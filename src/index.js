/* eslint no-param-reassign: ["error", {
  "props": true,
  "ignorePropertyModificationsFor": ["message", "response"]
}]
*/
...
module.exports = (config) => {
  ...
  function assertInputs(sendResponse) {
    if (!sendResponse) { throw new Error(errors.sendResponse.empty); }
    if (typeof sendResponse !== 'function') { throw new Error(errors.sendResponse.notAFn); }
  }
  ...
  async function processEvent(sendResponse, message) {
    ...
    const responseMessage = await exectuteAction(args, message);
  ...
  async function typeToObject(type) {
    ...
    if (!parts || parts.length < 2) { return pError(errors.message.type.badFormat); }
